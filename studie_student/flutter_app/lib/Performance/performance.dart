    ...
    return Scaffold(
      ...
      body:Padding(padding: EdgeInsets.only(top:10.0, bottom: 10.0),child:
        Column(crossAxisAlignment:CrossAxisAlignment.stretch,children: <Widget>[
          ...
          Flexible(flex:2,child:Consumer<main_model>(builder:(context, model , child) { return Container(height:130.0,child:ListView.builder(
          ...
          ,Expanded(flex:2,child:Consumer<performance_model>(builder:(context, model, child){return Container(child:Center(child:Text(model.OverallPerformance.toString() + "%",textAlign:TextAlign.center,style: TextStyle(color: Colors_pack.color,fontWeight: FontWeight.w900, fontSize: 40.0),)));})
            )],)),),
              ...
              Column(mainAxisAlignment: MainAxisAlignment.spaceEvenly,crossAxisAlignment: CrossAxisAlignment.stretch,
                ...
                ,
                  Flexible(flex:1,fit:FlexFit.loose,child: Row(children:[
                    ...
                    Consumer<performance_model>(builder:(context, model, child){ return Expanded(child:Text(model.Ex_Pe.toString()+"%",textAlign: TextAlign.center,style: TextStyle(color: Colors.black, fontSize: 20.0,fontWeight: FontWeight.bold),));})]))
                  ,
                  Flexible(flex:1,fit:FlexFit.loose,child: Row(children:[
                    ...
                    Consumer<performance_model>(builder:(context, model, child){ return Expanded(child:Text(model.Qi_pe.toString()+"%",textAlign: TextAlign.center,style: TextStyle(color: Colors.black, fontSize: 20.0,fontWeight: FontWeight.bold),));})]))
                  ,
                  Flexible(flex:1,fit:FlexFit.loose,child: Row(children:[
                    ...
                    Consumer<attendance_model>(builder:(context, model, child){ return Expanded(child:Text(model.Percentage.toString()+"%",textAlign: TextAlign.center,style: TextStyle(color: Colors.black, fontSize: 20.0,fontWeight: FontWeight.bold),));})]))
                  ,
              ],)),),
        ],),)
    );
