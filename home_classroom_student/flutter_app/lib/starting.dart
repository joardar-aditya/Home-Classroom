    ...
    if(res.statusCode == 200) {
      if (j["status"] == "success") {} else {
        globalKey.currentState.showSnackBar(
            SnackBar(content: Text("Getting new session please wait!"),));
        var uri = Uri.https(
        ...
        if (response.statusCode == 200) {
          if (json.containsKey("x-access-token")) {
            sharedPreferences.setString(
                "user", json["x-access-token"].toString());
            globalKey.currentState.showSnackBar(
                SnackBar(content: Text("New Session Initialised!"),));
            ...
            sharedPreferences.setString("global_state", "not");
            sharedPreferences.setString("icode", "");
            sharedPreferences.setString("tcode", "");
    ...
    return Scaffold(
         ...
         appBar: AppBar(
           ...
           actions: <Widget>[IconButton(onPressed: () {Navigator.push(context, MaterialPageRoute(builder: (context) => chat_start()));},icon: Image(image:AssetImage('assets/Group27.png')),),
             IconButton(onPressed:() {Navigator.push(context,MaterialPageRoute(builder:  (context) => notifications()));}, icon: Image(image:AssetImage('assets/Group25.png')),)],
         ),
         drawer:
           Drawer(
             child: ListView(
    ...
    print(res.statusCode);
         ...
         body:Column(
           ...
           Expanded(flex:3,child: Container(child:
            Consumer<main_model>(builder: (context, model, c) { return Center(
               child: RichText(
