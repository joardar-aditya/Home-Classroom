...
import 'package:studieteacher/chat/chat_groups.dart';
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar:  AppBar(
        ...
        actions: <Widget>[Container(margin:EdgeInsets.all(5),child:
        RaisedButton(shape:CircleBorder(side: BorderSide.none),
            color:Colors_pack.color, elevation:0.0,
            onPressed:() {setState(() {
              search = !search;
              ...
              return InkWell(
                  onTap: () {
                    setState(() {
                      ...
                      if(current_pressed == "Groups"){
                        groups = true;
                      }else if(current_pressed == "Student"){
                        groups = false;
                      }else if(current_pressed == "Parents"){
                        groups = false;
                      }else{
                        groups = false;
                      }
    ...
    Text("students", style:TextStyle(color:Colors.black, fontSize: 18))
          ...
          (groups)?chat_group():Consumer<chat_model>(builder:(context, model, child){ return Container(
            margin: EdgeInsets.only(top:20),
            child: (model.students.isEmpty)?Container(child: Container(child: CircularProgressIndicator(),),):ListView.builder(shrinkWrap:true, physics:NeverScrollableScrollPhysics(),itemCount:model.students.length,itemBuilder: (context, ind) {
              return (!search)?InkWell(
                  onTap: () {
                  ...
                  children: <Widget>[
                    ...
                    Column(
                      ...
                      children: <Widget>[
                        ...
                        (model.students[ind].Role == "parents")?Text(""):Text("Class " + model.students[ind].Class + " Sec " + model.students[ind].section, style:TextStyle(color:Colors.black, fontSize: 18))
                      ],
                    )
                  ],
                ),
              )):(search && model.students[ind].Name.toLowerCase().startsWith(se.toLowerCase()))?InkWell(
                  onTap: () {
                    Navigator.push(context, MaterialPageRoute(builder: (context) => chat_details(model.students[ind])));
                  },
                  child:Container(
