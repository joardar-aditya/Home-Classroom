...
import 'package:studieteacher/chat/Messages.dart';
...
class _stateI extends State<chat_details> {
  ...
  void getMessages() async {
    ...
    if (current == null) {
      updates = await ref
          .child("/teacher/" + teacher + "_" + school + "/contacts/students/anonymous/messages/")
          .orderByChild("sent_date")
          .onValue
          .listen((event) {
        Map mes = event.snapshot.value;
        List<Messages> curren = [];
        for (int i = 0; i < mes.keys
            .toList()
            .length; i++) {
          Map c = mes[mes.keys.toList()[i]];
          print(c.toString());
          curren.add(Messages(
              c["text"].toString(), c["journ"].toString(), c["status"],
              c["sent_date"]));
        }
        setState(() {
      ...
      updates = await ref
          .child("/teacher/" + teacher + "_" + school + "/contacts/"+current.Role+"/" +
          current.code + "_" + school + "/messages/")
          .orderByChild("sent_date")
          .onValue
          .listen((event) {
        Map mes = event.snapshot.value;
  ...
  Widget build(BuildContext context) {
    ...
    appBar: AppBar(
      ...
      title: Column(
        ...
        children: <Widget>[
          Text((current==null)?"Anonymous":current.Name, maxLines: 1, style: TextStyle(fontWeight: FontWeight.bold, fontSize: 20),),
          Text((current==null)?"student":(current.Role=="parents")?"Parent":"Class " + current.Class+  " Sec " + current.section, maxLines: 1, style: TextStyle(fontSize: 16),)

        ],
      ),
      actions: <Widget>[Container(margin:EdgeInsets.all(5),child:RaisedButton(color:Colors.white, elevation:0.0, onPressed:() {},child:Text('...', style: TextStyle(fontSize:36,fontWeight: FontWeight.w900, color: Colors_pack.color),)))],
    ),
    body: Column(
          ...
          InkWell(
              onTap: () async{
                ...
                if(current==null){
                  showDialog(
                    ...
                    child: Dialog(
                      ...
                      child: Container(
                        ...
                        child: Text("Cannot reply to Anonymous message!", style: TextStyle(color: Colors.white),),
                      ),
                    )
                  );
                }else {
                  ...
                  ins.setPersistenceEnabled(true);
                  final ref = ins.reference();
                  DatabaseReference dat = await ref.child("/teacher/" +
                      teacher + "_" + school + "/contacts/"+current.Role+"/" +
                      current.code + "_" + school + "/messages/");
                  String id = dat
                      .push()
                      .key;
                  Messages m = new Messages(controller.text, "rec", 0, DateTime
                      .now()
                  ...
                  DatabaseReference dat_2 = await ref.child("/${current.Role}/" +
                      current.code + "_" + school + "/contacts/teachers/" +
                      teacher + "_" + school + "/messages/");
                  String id_2 = dat
                      .push()
                      .key;
                  dat_2.child(id).set({
