...
class _assignState extends State<assign> {
  ...
  final List<String> _classes = List<String>.generate(11, (index) => (index + 1).toString()).toList();
  ...
  final List<String> _year = List<String>.generate(50, (i) => (2000+i).toString()).toList(growable: false);
  ...
  void initState() {
    ...
    if(["JAN","MAR", "MAY", "JUL", "AUG", "OCT", "DEC"].contains(_submitMonth)){
      day_list = List<String>.generate(31, (i) => (1+i).toString()).toList(growable: false);
    }else if (["APR","JUN", "SEP", "NOV"].contains(_submitMonth)) {
      day_list = List<String>.generate(30, (i) => (1+i).toString()).toList(growable: false);

    }else {
      if (int.parse(_submityear) % 4 == 0) {
        day_list =
            List<String>.generate(29, (i) => (1 + i).toString()).toList(
                growable: false);
      } else {
        day_list =
            List<String>.generate(28, (i) => (1 + i).toString()).toList(
                growable: false);
      }}
  ...
  void _changeYear(String value){setState(() {
    _submitday = "1";
    _submityear = value;
  });
  }
  ...
  Widget build(BuildContext context) {
    ...
    Text('Choose a Chapter', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 22),),),
    Container(decoration: BoxDecoration(
    color: Color(0xff261FFF),
    borderRadius: BorderRadius.circular(5)
    ),padding:EdgeInsets.only(left:10),child:
    ...
    value: item,
        ...
        Container(margin:EdgeInsets.symmetric(vertical: 10, horizontal: 20),child:
        Text('Assign a homework', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 22),),),
        Container(
          decoration: BoxDecoration(
          ...
          child:Align(alignment:Alignment.centerLeft,child:TextFormField(
            decoration: InputDecoration(border:InputBorder.none,focusColor: Colors.grey,fillColor: Colors.grey, labelText: 'Add a title', labelStyle: TextStyle(color: Colors.pinkAccent)),
            autocorrect: true,
            maxLines: 1,
          )
        ...
        Container(
          margin: EdgeInsets.symmetric(vertical: 10, horizontal: 20),
          child: FittedBox(fit:BoxFit.contain,child:Row(
            mainAxisAlignment: MainAxisAlignment.start,
            mainAxisSize: MainAxisSize.min,
            children: <Widget>[
             Flexible(flex:3,fit:FlexFit.loose,child:
              Container(width: 300,
                  decoration: BoxDecoration(
                    color: Colors.grey[300],
                    borderRadius: BorderRadius.circular(20),
                  ...
                  margin: EdgeInsets.symmetric(vertical: 10, horizontal: 20),
                  child:Align(alignment:Alignment.centerLeft,child:TextFormField(
                    decoration: InputDecoration(border:InputBorder.none,focusColor: Colors.grey,fillColor: Colors.grey, labelText: 'Add description', labelStyle: TextStyle(color: Colors.pinkAccent)),
                    autocorrect: true,
                    maxLines: 3,
                  )

                  ))),
            Flexible(flex:1,fit:FlexFit.loose,child:
            Container(child:
              RaisedButton(disabledColor:Colors_pack.color,color:Colors_pack.color,child:Container(child: Center(child:Icon(Icons.add, color: Colors.white,size: 50,)),),shape: CircleBorder(side: BorderSide.none),))
            )],
          )
        )),
        Container(
          ...
          child: Row(
              ...
              children: <Widget>[Column(
                    ...
                    selectedItemBuilder: (BuildContext context) {
                      return day_list.map<Widget>((String item) {
                        return Container(
                            ...
                            child:Center(child: Text(item.toString(), style:TextStyle(fontWeight: FontWeight.bold,fontSize: 20, color: Colors.black),),
                            ));
                      }).toList();
                    },
                ...
                Column(
                  children:[Container(child:
                  Text('Month', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 22),),),
                    Padding(padding: EdgeInsets.symmetric(horizontal: 5),child:
                    DropdownButton<String>(
                      value: _submitMonth,
        ...
        Column(crossAxisAlignment:CrossAxisAlignment.center,children:[
          Container(
              ...
              child:RaisedButton(onPressed: () {},
                  ...
                  child:Padding(padding:EdgeInsets.all(10),child:Text('Done', textAlign: TextAlign.center, style: TextStyle(fontSize:22,fontWeight:FontWeight.bold, color:Colors.white
                  )))))])
